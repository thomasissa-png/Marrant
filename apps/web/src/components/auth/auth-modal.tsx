...
export function AuthModal({ isOpen, onClose, defaultTab = "login" }: AuthModalProps) {
  ...
  const handleApple = () => {
    signIn("apple", { callbackUrl: tab === "register" ? "/onboarding" : "/" });
  };
        ...
        <CardContent>
          {tab === "login" ? (
            <form className="flex flex-col gap-4" onSubmit={handleLogin}>
              ...
              <Button type="button" variant="outline" className="w-full" onClick={handleApple}>
                <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24" fill="currentColor"><path d="M17.05 20.28c-.98.95-2.05.88-3.08.4-1.09-.5-2.08-.52-3.23 0-1.44.62-2.2.44-3.06-.4C3.79 16.17 4.36 9.02 8.9 8.75c1.27.06 2.15.72 2.91.76.97-.2 1.9-.9 2.96-.82 1.26.1 2.2.6 2.82 1.5-2.58 1.55-1.97 4.96.58 5.92-.46 1.2-.68 1.73-1.28 2.78-.84 1.47-2.03 2.94-3.84 1.39zM12.97 8.63c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/></svg>
